...
apt update -y
# 安装所需工具
apt-get install socat cron curl wget gnupg unzip iputils-ping apt-transport-https ca-certificates software-properties-common gawk p7zip-full systemd-timesyncd iptables zstd -y
# 创建目录
...
function install_ols {
    ...
    ./install.sh
    # 安装PHP 和 扩展
    apt install lsphp${PHP_VERSION} lsphp${PHP_VERSION}-common lsphp${PHP_VERSION}-intl lsphp${PHP_VERSION}-curl lsphp${PHP_VERSION}-opcache lsphp${PHP_VERSION}-imagick lsphp${PHP_VERSION}-mysql lsphp${PHP_VERSION}-memcached -y
    # 删除其他PHP
    [ -f /usr/bin/php ]  && rm -f /usr/bin/php
    # 创建PHP软链接
    ln -s $OLS_ROOT/lsphp${PHP_VERSION}/bin/php /usr/bin/php
    cd $SRC_PATH && rm -rf *
    # 下载配置文件
